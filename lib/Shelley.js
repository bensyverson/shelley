...
Shelley.prototype.linkMarkdown = function(text, doc) {
	...
	return text.replace(self.mdLinkRegex, function(match, name, path, file){
		console.log("⭐️ Looking for " + path + file)
		const linkedDoc = self.fileAt(path, file, self.documents, doc.file.path)
		if (!linkedDoc) { console.log("Not found."); return match }

		console.log("• Path from " + doc.path.join('/') + " to " + linkedDoc.path.join('/') + ": " + self.pathTo(linkedDoc.path, doc.path))
		 return '[' + name + '](' + self.pathTo(linkedDoc.filePath('', '.html').split('/'), doc.filePath('', '.html').split('/')) + ')'
...
Shelley.prototype.pathTo = function(path, relativeToPath) {
	...
	let toPath = path.slice()
	let fromPath = relativeToPath.slice(0,-1)
	while (fromPath[0] == toPath[0]) {
		fromPath = fromPath.slice(1)
		toPath = toPath.slice(1)
	}

	return Array(fromPath.length)
		.fill('..')
		.concat(toPath)
		.join('/')
}
...
Shelley.prototype.fileAt = function(path, file, inArray, relativePath) {
	...
	let pathArray = path.split('/').filter(function(el){ return el != '' })

	console.log("   Original path: " + pathArray.join('/') + ", relative path: " + relativePath.join('/'))

	while (pathArray[0] == '..') {
		pathArray = pathArray.slice(1)
		relativePath.pop()
	}
	pathArray = relativePath.concat(pathArray)

	console.log("   File path: " + pathArray.join('/') + ", relative path: " + relativePath.join('/'))
