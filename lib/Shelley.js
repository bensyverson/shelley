...
const Shelley = function(options){
	...
	this.mdLinkRegex = new RegExp(/href\s*=\s*"(?:([\w]+)+\/)*(\/|[\w-]+(?:\.[\w]+)?)?"/gi)
...
Shelley.prototype.buildDocument = function(doc) {
	...
	doc.body = self.linkMarkdown( doc.contents, doc )
...
Shelley.prototype.fileAt = function(path, file, inArray) {
	const self = this
	const pathArray = path.split('/').filter(function(el){ return el != '' })
	return inArray.filter(function(asset){
		return Utils.areArraysEqual(pathArray, asset.path) && asset.filename == file
	})[0]
}
